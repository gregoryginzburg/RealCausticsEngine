...
float fresnel_dielectric_eta(float cos_theta, float eta)
{
	...
	float g = eta * eta - 1.0f + cos_theta * cos_theta;
	float result;
...
Specular_Reflection::Specular_Reflection(const vec3& color, const Isect& intersection, TransportMode m) : R(color), mode(m)
{
	...
	mContiniationProb = std::max(std::max(color.x, color.y), color.z);
}
...
vec3 Specular_Reflection::sample(vec3& wi, const vec3& geometric_normal, float& forward_pdf, float& reverse_pdf, float& cos_theta_out, float& contination_prob, BxDFType& sampled_event,
...
vec3 Specular_Transmission::sample(vec3& wi, const vec3& geometric_normal, float& forward_pdf, float& reverse_pdf, float& cos_theta_out, float& contination_prob, BxDFType& sampled_event,
	MLT_Sampler& Sampler) const
{
	Sampler.skip(3);
	...
	vec3 wi_local;
	if (!refract(wo_local, wo_local.z > 0 ? frame.mZ : -frame.mZ, etai_over_etat, wi_local))
		return 0.0f;
...
vec3 Fresnel_Specular::sample(vec3& wi, const vec3& geometric_normal, float& forward_pdf, float& reverse_pdf, float& cos_theta_out, float& contination_prob, BxDFType& sampled_event,
	MLT_Sampler& Sampler) const
{
	...
	float u = sample.x;
	float f = fresnel_dielectric_eta(fabs(wo_local.z), etai_over_etat);
...
vec3 Lambertian_Reflection::sample(vec3& wi, const vec3& geometric_normal, float& forward_pdf, float& reverse_pdf, float& cos_theta_out, float& contination_prob, BxDFType& sampled_event,
	MLT_Sampler& Sampler) const
{
	vec3 sample = Sampler.Get3D();
	...
	vec3 local_out = sample_cos_hemisphere(forward_pdf, e);
	cos_theta_out = local_out.z;

	wi = frame.to_world(local_out);
...
// a = original roughness (dont need to sqare it again)
	...
	MLT_Sampler& Sampler) const
{
	...
	forward_pdf = reverse_pdf = d * cos_h_n * 0.25f / dot(half_vector, wo_local);

	contination_prob = mContiniationProb;

	float g2 = G2(fabs(wo_local.z), fabs(wi_local.z), a2);
	return R * g2 * abs_dot(wo_local, half_vector) / (fabs(wo_local.z) * fabs(half_vector.z));
}
...
vec3 Microfacet_Reflection_GGX::evaluate(const vec3& wi, const vec3& geometric_normal) const
{
	...
	return R * d * g2 * 0.25f / (fabs(wo_local.z) * fabs(wi_local.z));
}
	...
	MLT_Sampler& Sampler) const
{
	...
	if (u < f)
	{
		...
		reflectance = g2 * abs_dot(wo_local, half_vector) / (fabs(wo_local.z) * fabs(half_vector.z));
	...
	else
	{
		...
		vec3 wi_local = sample_cos_hemisphere(forward_pdf, e);
		cos_theta_out = fabs(wi_local.z);
		...
		float D90 = 0.5f + fabs(h.z) * a2 - 1.0f;

		contination_prob = (R * (1.0 - f)).Max();

		reflectance = R * (1.0f + D90 * pow5(1.0f - fabs(wo_local.z))) * (1.0f + D90 * pow5(1.0f - fabs(wi_local.z)));
	}
...
vec3 Fresnel_Microfacet_GGX::evaluate(const vec3& wi, const vec3& geometric_normal) const
{
	...
	vec3 diffuse = R * (1.0f + D90 * pow5(1.0f - fabs(wo_local.z))) * (1.0f + D90 * pow5(1.0f - fabs(wi_local.z))) * (1.0f - f) * fabs(wi_local.z) * Inv_PI;
...
void Fresnel_Microfacet_GGX::pdf(const vec3& wi, float& forward_pdf, float& reverse_pdf) const
{
	...
	float forward_specular_pdf = d * cos_h_n * 0.25f / dot(half_vector, wo_local);
